...
def_top = def_topp*p2mm*0.001;            % Deflection of top (pixels)
def_bot = def_botp*p2mm*0.001;            % Deflection of bottom (pixels)

% Force calculations assuming cantilever connection
P_top = (def_top*3*E_mod*I_strip)/((L_strip)^3);
P_bot = (def_bot*3*E_mod*I_strip)/((L_strip)^3);
...
xline(start_off, ":", "Offset start", ...
    "LineWidth", 1, "Color", "yellow")
xline(tip, ":", "Jaw Tips", "LineWidth", 1, "Color", "yellow")
text(tip - 250, max(dend_y) + 20, "Bottom Clamp Force (N): " + P_bot, ...
    "Color", "g", "FontSize", 14, "FontWeight", 'bold');
text(tip - 250, min(dend_y) - 20, "Top Clamp Force (N): " + P_top, ...
    "Color", "g", "FontSize", 14, "FontWeight", 'bold');
hold off
